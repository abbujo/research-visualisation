...
var instructionText =
  "Click on nodes to <b>find related entities</b>. CTRL+click to <b>go to Research Wiki</b>. SHIFT+click to <b>collapse nodes</b>.";
var retrievalText = "Retrieving data. Please wait...";
var noMoreDataText = "No more data found...";
query1 = (parent) =>
  '{ Subject(filter: { _id:"' +
  parent +
  '"}) { _id Reduce { _id _type label } } }';

query2 = (topic) =>
  "{ _CONTEXT { _id _type Subject label } Subject(filter:{_id: [" +
  topic.map(function (item) {
    return '"' + item + '"';
  }) +
  "]}){ _id _type label Reduce { _id }} } ";
...
function init(uri) {
  ...
  var body = JSON.stringify({
    query: '{ Subject(filter:{_id: "' + uri + '"}){ _id _type label } }',
  });
